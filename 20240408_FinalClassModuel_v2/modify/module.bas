...
Public Sub Main()
    ...
    ' 空白報表路徑
    gReportFolder = ThisWorkbook.Path & "\" & Sheets("ControlPanel").Range("EmptyReportPath").Value
    ' 產生之申報報表路徑
    gOutputFolder = ThisWorkbook.Path & "\" & Sheets("ControlPanel").Range("OutputReportPath").Value
    ' 製作報表List
...
Public Sub Process_FB1()
    ...
    Else
        ...
        Debug.Print reportTitle & ": " & queryTable & " 資料表有資料，此表單尚無有資料紀錄，尚請確認。"
    End If
...
Public Sub Process_FB3A()
    ...
    xlsht.Range("K:Q200").ClearContents
    ...
    If (lastRow > 1) Then
        ...
        For i = 2 To lastRow
            ...
            AddDynamicField "FOA", "FB3A_BankCode", "C" & CStr(targetRow), CStr(BankCode)
            AddDynamicField "FOA", "FB3A_Amount", targetCol & CStr(targetRow), CStr(Amount)
            
            InsertIntoTable gDBPath, "MonthlyDeclarationReport", gDataMonthString, "FB3A", "FOA|FB3A_BankCode", CStr(BankCode), "C" & CStr(targetRow)
            InsertIntoTable gDBPath, "MonthlyDeclarationReport", gDataMonthString, "FB3A", "FOA|FB3A_Amount", CStr(Amount), targetCol & CStr(targetRow)
...
Public Sub Process_Table2()
    ...
    rpt.SetField "FOA", "Table2_其他", CStr(marginDeposit_TWD)

    xlsht.Range("Table_美元_F1").Value = marginDeposit_USD
    rpt.SetField "FOA", "Table_美元_F1", CStr(marginDeposit_USD)

    xlsht.Range("Table2_美元_F3").Value = rateUSDtoTWD
    rpt.SetField "FOA", "Table2_美元_F3", CStr(rateUSDtoTWD)

    xlsht.Range("Table2_美元_F4").Value = marginDeposit_TWD
    rpt.SetField "FOA", "Table2_美元_F4", CStr(marginDeposit_TWD)
...
Public Sub Process_FM2()
    ...
    For i = 2 To lastRow
        ...
        If (Not IsEmpty(xlsht.Cells(i, "P").Value)) Then
            ...
            AddDynamicField "FOA", "FM2_BankCode", "C" & CStr(targetRow), CStr(BankCode)
            AddDynamicField "FOA", "FM2_Amount", targetCol & CStr(targetRow), CStr(Amount) 
            
            InsertIntoTable gDBPath, "MonthlyDeclarationReport", gDataMonthString, "FM2", "FOA|FM2_BankCode", CStr(BankCode), "C" & CStr(targetRow)
            InsertIntoTable gDBPath, "MonthlyDeclarationReport", gDataMonthString, "FM2", "FOA|FM2_Amount", CStr(Amount), targetCol & CStr(targetRow)
...
Public Sub Process_F1_F2()
    ...
    xlsht.Range("AB2:AB100").ClearContents
    xlsht.Range("AF2:AF100").ClearContents
    xlsht.Range("AJ2:AJ100").ClearContents
    xlsht.Range("AN2:AN100").ClearContents
    xlsht.Range("AR2:AR100").ClearContents
    ...
    ' Cur 在前一欄, Value 在後一欄
    dataRanges = Array("A:B", "C:D", "E:F", "G:H", "Q:R") 
    ...
    For i = LBound(transactionTypes) To UBound(transactionTypes)
        ...
        ' 取得起始欄位（Cur欄）
        currCol = xlsht.Range(dataRanges(i)).Column
        ...
        For j = LBound(currencies) To UBound(currencies)
            ...
            ' 產生field名稱
            fieldName = transactionTypes(i) & "_" & currencies(j) 
    ...
    xlsht.Range("AB2:AB100").NumberFormat = "#,##,##.0"
    xlsht.Range("AF2:AF100").NumberFormat = "#,##,##.0"
    xlsht.Range("AJ2:AJ100").NumberFormat = "#,##,##.0"
    xlsht.Range("AN2:AN100").NumberFormat = "#,##,##.0"
    xlsht.Range("AR2:AR100").NumberFormat = "#,##,##.0"
...
Public Sub UpdateExcelReports()
    ...
    For Each rptName In gReportNames
        ...
        emptyFilePath = gReportFolder & "\" & Sheets("ControlPanel").Range(rptName & "_FileName").Value & "." & Sheets("ControlPanel").Range(rptName & "_ExtensionName").Value
        outputFilePath = gOutputFolder & "\" & Sheets("ControlPanel").Range(rptName & "_FileName").Value & gDataMonthString & "." & Sheets("ControlPanel").Range(rptName & "_ExtensionName").Value
