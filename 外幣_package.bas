...
Sub ProcessAllReports()
    ...
    For i = LBound(rptFolders) To UBound(rptFolders)
        If Dir(savePdfRoot & "\" & rptFolders(k), vbDirectory) = "" Then
            MkDir savePdfRoot & "\" & rptFolders(k)
        End If
    Next i
...
Private Sub Import_AccessCreate(ByVal basePath   As String, _
    ...
    '— 路徑置換 ——
    If Left(tplPattern, 3) = "..\" Then        
        oldTplRel = Replace(Replace(tplPattern, "..\", ""), "YYYYMM", westernMont)
        newTplRel = Replace(Replace(tplPattern, "..\", ""), "YYYYMM", newMon)
        tplPath     = ParentFolder(basePath) & "\" & oldTplRel
        declTplPath = basePath & "\" & newTplRel
    ElseIf
        oldTplRel = Replace(tplPattern, "YYYYMM", oldMon)
        newTplRel = Replace(tplPattern, "YYYYMM", westernMont)
    ...
    baseFile = Mid(newTplRel, InStrRev(newTplRel, "\") + 1)
    ...
    saveRoot = ThisWorkbook.Path & "\SAVE_PDF\" & newMon & "\" & rptID
    If Dir(saveRoot, vbDirectory) = "" Then
        MkDir ThisWorkbook.Path & "\SAVE_PDF\" & newMon ' 上層 (如不存在)
        MkDir saveRoot
    End If
    ...
    For j = LBound(arrPdfSheets) To UBound(arrPdfSheets)
        ...
        If shName <> "" Then
            ...
            If Not wsToExport Is Nothing Then
                ...
                pdfName = saveRoot & fileNoExt & "_" & Replace(shName, "/", "_") & ".pdf"
...
Private Sub Import_外幣債評估表及減損表(ByVal basePath   As String, _
    ...
    For j = LBound(arrPdfSheets) To UBound(arrPdfSheets)
        ...
        If shName <> "" Then
            On Error Resume Next
            Set wsToExport = wbOld.Worksheets(shName)
            On Error GoTo 0
            If Not wsToExport Is Nothing Then
                Dim pdfName As String
                pdfName = saveRoot & fileNoExt & "_" & Replace(shName, "/", "_") & ".pdf"
