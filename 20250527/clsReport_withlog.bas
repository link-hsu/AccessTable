...
Public Sub Init(ByVal reportName As String, _
    ...
    WriteLog ">>> Init 開始: reportName=" & reportName
    ...
    Set wsFields = CreateObject("Scriptiji3ung.Dictionary")

    If Not IsArray(addressMap) Then
        WriteLog "!!! GetMapData 回傳非 Array"
    ElseIf UBound(addressMap,1) < 0 Then
        WriteLog "!!! GetMapData 回傳空陣列"
    Else
        WriteLog "GetMapData 共取回 " & (UBound(addressMap,1)+1) & " 筆 mapping"
    End If
    ...
    If IsArray(addressMap) And UBound(addressMap) >= 0 Then
        ...
        For i = 0 To UBound(addressMap, 1)
            ...
            ' 先 log 原始值，特別注意 Null
            WriteLog "  Row " & i & _
            "  rawSheet=" & IIf(IsNull(sheetName), "<Null>", sheetName) & _
            "  rawTag="   & IIf(IsNull(nameTag),   "<Null>", nameTag) & _
            "  rawAddr="  & IIf(IsNull(addr),  "<Null>", addr)

            ' 再做 Null 濾掉或轉 ""
            If IsNull(nameTag) Then nameTag = ""
            If IsNull(addr) Then addr = ""
            nameTag = CStr(nameTag)
            addr    = CStr(addr)
            
            ' 再 log 轉型後
            WriteLog "    轉型後 nameTag=""" & nameTag & """  addr=""" & addr & """"
