...
Public Sub Process_CNY1()
    ...
    If (lastRow > 1) Then
        ...
        fxReceive = ABs(RoundUp(fxReceive / 1000, 0))
        fxPay = ABs(RoundUp(fxPay / 1000, 0))
    End If
...
Public Sub Process_FB3()
    ...
    If (lastRow > 1) Then
        ...
        assetTW = RoundUp(assetTW / 1000, 0)
        liabilityTW = RoundUp(liabilityTW / 1000, 0)
...
Public Sub Process_FB3A()
    ...
    If (lastRow > 1) Then
        ...
        For i = 2 To lastRow
            ...
            ' 金額
            Amount = RoundUp(xlsht.Cells(i, "H").value / 1000, 0)
            ' 類別 
...
Public Sub Process_FM11()
    ...
    foreignInterestRevenue = RoundUp(foreignInterestRevenue / 1000, 0)
    gainOnSecuritiesDisposal = RoundUp(gainOnSecuritiesDisposal / 1000, 0)
    lossOnSecuritiesDisposal = RoundUp(lossOnSecuritiesDisposal / 1000, 0)
    reversalImpairmentPL = RoundUp(reversalImpairmentPL / 1000, 0)
    valuationImpairmentLoss = RoundUp(valuationImpairmentLoss / 1000, 0)
    domesticInterestRevenue = RoundUp(domesticInterestRevenue / 1000, 0)
...
Public Sub Process_FM13()
    ...
    If (lastRow > 1) Then
        ...
        bookValue_HongKong = RoundUp(bookValue_HongKong / 1000, 0)
        bookValue_Korea = RoundUp(bookValue_Korea / 1000, 0)
        bookValue_Thailand = RoundUp(bookValue_Thailand / 1000, 0)
        bookValue_Malaysia = RoundUp(bookValue_Malaysia / 1000, 0)
        bookValue_Philippines = RoundUp(bookValue_Philippines / 1000, 0)
        bookValue_Indonesia = RoundUp(bookValue_Indonesia / 1000, 0)
        
        valueAdjsut = ABs(RoundUp(valueAdjsut / 1000, 0))
        accumulateImpairment = RoundUp(accumulateImpairment / 1000, 0)
    End If
...
Public Sub Process_AI821()
    ...
    If (lastRow > 1) Then       
        ...
        domesticBank = RoundUp(domesticBank, 0)
        chinaBranchBank = RoundUp(chinaBranchBank, 0)
        foreignBranchBank = RoundUp(foreignBranchBank, 0)
        chinaBank = RoundUp(chinaBank, 0)
        others = RoundUp(others, 0)
    End If
...
Public Sub Process_Table2()
    ...
    If (lastRow > 1) Then
        ...
        marginDeposit_USD = RoundUp((marginDeposit_TWD / rateUSDtoTWD) / 1000, 0)
        marginDeposit_TWD = RoundUp(marginDeposit_TWD / 1000, 0)
...
Public Sub Process_FB5()
    ...
    SpotToDBU_CNY = RoundUp(SpotToDBU_CNY / 1000, 0)
...
Public Sub Process_FM2()
    ...
    For i = 2 To lastRow
        ...
        If (Not IsEmpty(xlsht.Cells(i, "P").Value)) Then
            ...
            ' 金額
            Amount = RoundUp(xlsht.Cells(i, "O").value / 1000, 0)
            ' 類別 
...
Public Sub Process_FM10()
    ...
    FVPL_VALUE = RoundUp(FVPL_VALUE / 1000, 0)
    FVPL_NET_VALUE = RoundUp(FVPL_NET_VALUE / 1000, 0)
    FVOCI_VALUE = RoundUp(FVOCI_VALUE / 1000, 0)
    FVOCI_NET_VALUE = RoundUp(FVOCI_NET_VALUE / 1000, 0)
    AC_VALUE = RoundUp(AC_VALUE / 1000, 0)
    AC_NET_VALUE = RoundUp(AC_NET_VALUE / 1000, 0)
    otherFinancialAssets = RoundUp(otherFinancialAssets / 1000, 0)
...
Public Sub Process_F1_F2()
    ...
    For i = LBound(transactionTypes_F1) To UBound(transactionTypes_F1)
        ...
        For j = 2 To lastRow ' 假設第1列是標題，從第2列開始
            ...
            ' 貨幣數值 百萬元，四捨五入小數第一位
            curValue = RoundUp(xlsht.Cells(j, currCol + 1).Value / 1000000, 1) 
...
Public Sub Process_Table41()
    ...
    derivativeGain = RoundUp(derivativeGain / 1000, 0)
    derivativeLoss = ABs(RoundUp(derivativeLoss / 1000, 0))
...
Public Sub Process_AI602()
    ...
    FVPL_GovDebt_Cost = RoundUp(FVPL_GovDebt_Cost / 1000, 0)
    FVPL_GovDebt_BookValue = RoundUp(FVPL_GovDebt_BookValue / 1000, 0)
    FVOCI_GovDebt_Cost = RoundUp(FVOCI_GovDebt_Cost / 1000, 0)
    FVOCI_GovDebt_BookValue = RoundUp(FVOCI_GovDebt_BookValue / 1000, 0)
    AC_GovDebt_Cost = RoundUp(AC_GovDebt_Cost / 1000, 0)
    AC_GovDebt_BookValue = RoundUp(AC_GovDebt_BookValue / 1000, 0)
    ...
    FVPL_CompanyDebt_Cost = RoundUp(FVPL_CompanyDebt_Cost / 1000, 0)
    FVPL_CompanyDebt_BookValue = RoundUp(FVPL_CompanyDebt_BookValue / 1000, 0)
    FVOCI_CompanyDebt_Cost = RoundUp(FVOCI_CompanyDebt_Cost / 1000, 0)
    FVOCI_CompanyDebt_BookValue = RoundUp(FVOCI_CompanyDebt_BookValue / 1000, 0)
    AC_CompanyDebt_Cost = RoundUp(AC_CompanyDebt_Cost / 1000, 0)
    AC_CompanyDebt_BookValue = RoundUp(AC_CompanyDebt_BookValue / 1000, 0)
    ...
    FVPL_FinanceDebt_Cost = RoundUp(FVPL_FinanceDebt_Cost / 1000, 0)
    FVPL_FinanceDebt_BookValue = RoundUp(FVPL_FinanceDebt_BookValue / 1000, 0)    
    FVOCI_FinanceDebt_Cost = RoundUp(FVOCI_FinanceDebt_Cost / 1000, 0)
    FVOCI_FinanceDebt_BookValue = RoundUp(FVOCI_FinanceDebt_BookValue / 1000, 0)
    AC_FinanceDebt_Cost = RoundUp(AC_FinanceDebt_Cost / 1000, 0)
    AC_FinanceDebt_BookValue = RoundUp(AC_FinanceDebt_BookValue / 1000, 0)
...
Public Sub Process_AI822()
    ...
    CNPolicyBank_DepositAmount = RoundUp(CNPolicyBank_DepositAmount, 0)
    TWBranch_Loan_DepositAmount = RoundUp(TWBranch_Loan_DepositAmount, 0)
    TWBranch_Loan_LoanAmount = RoundUp(TWBranch_Loan_LoanAmount, 0)
