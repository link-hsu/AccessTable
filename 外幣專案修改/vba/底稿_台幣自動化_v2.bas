...
Private Sub Import_AccessCreate(ByVal basePath   As String, _
    ...
    '— 路徑置換 ——
    output_PDFTplRel = Replace(tplPattern, "YYYYMM", westernMonth)
    output_PDFTplRel = ParentFolder(basePath) & "\" & "OutputReport\" & output_PDFTplRel

    declTplRel = Replace(declTplRel, "YYYYMM", newMon)
    declTplPath = basePath & "\" & declTplRel

    FileCopy output_PDFTplRel, declTplPath

    '''*** MODIFIED START ***
    ' 開啟複製出來的檔案 (declTplPath)，並針對 pdfSheet（逗號分隔的分頁）逐一輸出 PDF
    
    Set wbNew = Workbooks.Open(Filename:=declTplPath, ReadOnly:=False)

    ' 取得 declTplPath 的最後檔名，並去掉副檔名，當作 base name
    Dim baseFile As String, fileNoExt As String
    baseFile = Mid(declTplPath, InStrRev(declTplPath, "\") + 1)
    If InStrRev(baseFile, ".") > 0 Then
        fileNoExt = Left(baseFile, InStrRev(baseFile, ".") - 1)
    Else
        fileNoExt = baseFile
    End If

    ' 確保儲存路徑存在（若 ProcessAllReports 已建立則這裡會跳過）
    Dim saveRoot As String
    saveRoot = basePath & "\SAVE_PDF\" & newMon & "\" & rptID & "\"


    ' 將 I 欄的分頁字串拆開，逐一輸出 PDF（檔名：<fileNoExt>_<sheetname>.pdf）
    Dim arrPdfSheets() As String
    arrPdfSheets = Split(pdfSheet, ",")
    Dim shName As String
    Dim wsToExport As Worksheet
    For j = LBound(arrPdfSheets) To UBound(arrPdfSheets)
        shName = Trim(arrPdfSheets(j))
        If shName <> "" Then
            ' On Error Resume Next
            Set wsToExport = wbNew.Worksheets(shName)
            ' On Error GoTo 0
            If Not wsToExport Is Nothing Then
                Dim pdfName As String
                pdfName = saveRoot & fileNoExt & "_" & shName & ".pdf"
                wsToExport.ExportAsFixedFormat Type:=xlTypePDF, _
                    Filename:=pdfName, _
            ...
            Else
                Debug.Print "Import_AccessCreate: 工作表 '" & shName & "' 在 " & declTplPath & " 找不到。"
            End If
            Set wsToExport = Nothing
        End If
    Next j
...
Private Sub Import_外幣債評估表及減損表(ByVal basePath   As String, _
                                      ByVal oldMon     As String, _
                                      ByVal newMon     As String, _
                                      ByVal rptID      As String, _
                                      ByVal tplPattern As String, _
    ...
    For j = LBound(impPatternArr) To UBound(impPatternArr)
        ...
        ElseIf impPatternArr(j) Like "*金融資產減損*" Then
            arrImpF(j) = Replace(impPatternArr(j), "YYYYMM", ROCMonthEnd)
        Else
            arrImpF(j) = Replace(impPatternArr(j), "YYYYMM", newMon)
        End If
    Next j
    ...
    Select Case rptId
        Case "FM13"
            ' 設定製作報表年月份
            wbOld.Sheets("FOA").Range("D2").Value = ROCYearMonth
    End Select
    ...
    ' 開啟複製出來的檔案 (declTplPath)，並針對 pdfSheet（逗號分隔的分頁）逐一輸出 PDF
    ' 取得 newTplRel 的最後檔名，並去掉副檔名，當作 base name
    Dim baseFile As String, fileNoExt As String
    baseFile = Mid(basePath & "\" & newTplRel, InStrRev(basePath & "\" & newTplRel, "\") + 1)
    If InStrRev(baseFile, ".") > 0 Then
        fileNoExt = Left(baseFile, InStrRev(baseFile, ".") - 1)
    Else
        fileNoExt = baseFile
    ...
    ' 將 J 欄的分頁字串拆開，逐一輸出 PDF（檔名：<fileNoExt>_<sheetname>.pdf）
    Dim arrPdfSheets() As String
    arrPdfSheets = Split(pdfSheet, ",")
    Dim shName As String
    Dim wsToExport As Worksheet
    For j = LBound(arrPdfSheets) To UBound(arrPdfSheets)
        shName = arrPdfSheets(j)
        If shName <> "" Then
            ' On Error Resume Next
            Set wsToExport = wbNew.Worksheets(shName)
            ' On Error GoTo 0
...
Private Sub Import_CopyThenRunAP(ByVal basePath   As String, _
                                 ...
                                 ByVal ROCYearMonth As String, _
                                 ByVal NUMYearMonth As String, _
                                 ByVal westernMonthEnd As String, _
                                 ByVal ROCMonth As String)
    ...
    Dim oldTplRel As String, newTplRel As String
    Dim tplPath   As String, declTplPath As String
    Dim declTplPathOld As String, declTplPathNew As String
    Dim j         As Long
    ...
    For j = LBound(impPatternArr) To UBound(impPatternArr)
        ...
        ElseIf impPatternArr(j) Like "*CloseRate*" Then
            ...
            Else
                Debug.Print "找不到 CloseRate 檔案：" & searchPath
            End If
        ElseIf impPatternArr(j) Like "*OBU_AC5411B*" Or _
               impPatternArr(j) Like "*DBU_DL9360*" Or _
               impPatternArr(j) Like "*OBU_MM4901B*" Or _
               impPatternArr(j) Like "*OBU_AC4603*" Or _
               impPatternArr(j) Like "*DBU_DL6850*" Or _
               impPatternArr(j) Like "*DBU_CM2810*" Then
            arrImpF(j) = Replace(impPatternArr(j), "YYYYMM", westernMonthEnd)            
        Else
            arrImpF(j) = Replace(impPatternArr(j), "YYYYMM", newMon)
        End If
    ...
    declTplPathOld = basePath & "\" & Replace(declTplRel, "YYYYMM", oldMon)
    declTplPathNew = basePath & "\" & Replace(declTplRel, "YYYYMM", newMon)
    ...
    Select Case rptId
        Case "FM11"
            ' 設定製作報表年月份
            wbOld.Sheets("FM11").Range("D3").Value = ROCYearMonth
        Case "表41"
            ' 設定製作報表年月份
            wbOld.Sheets("表41").Range("A3").Value = ROCYearMonth
        Case "FM2"
            ' 設定製作報表年月份
            wbOld.Sheets("FM2").Range("C2").Value = ROCYearMonth
        Case "FM10"
            ' 設定製作報表年月份
            wbOld.Sheets("FM10").Range("C2").Value = ROCYearMonth
        Case "F1_F2"
            ' 設定製作報表年月份
            wbOld.Sheets("F1").Range("B3").Value = ROCYearMonth
            wbOld.Sheets("F2").Range("B3").Value = ROCYearMonth
        Case "AI240"
            ' 設定製作報表年月份
    ...
    For j = LBound(arrImpF) To UBound(arrImpF)
        Select Case True
            ...
            Case Mid(arrImpF(j), InStrRev(arrImpF(j), "\") + 1) Like "*cm2610*"
                wbOld.Sheets(arrTplSh(j)).Range("A1:Z30").ClearContents
                Call ImportCloseRate(basePath & "\" & Trim(arrImpF(j)), wbOld.Sheets(arrTplSh(j)))            
            Case Mid(arrImpF(j), InStrRev(arrImpF(j), "\") + 1) Like "*DBU_CM2810*"
                ' wbOld.Sheets(arrTplSh(j)).Cells.ClearContents
                wbOld.Sheets(arrTplSh(j)).Range("A2:H5000").ClearContents
                Set wbImp = Workbooks.Open(basePath & "\" & Trim(arrImpF(j)), ReadOnly:=True)
            ...
            Case Mid(arrImpF(j), InStrRev(arrImpF(j), "\") + 1) Like "*DBU_DL6850*"
                ' wbOld.Sheets(arrTplSh(j)).Cells.ClearContents
                Set wbImp = Workbooks.Open(basePath & "\" & Trim(arrImpF(j)), ReadOnly:=True)

                If rptID = "AI240" Then
                    wbOld.Sheets(arrTplSh(j)).Range("B2:M5000").ClearContents
                    With wbOld.Sheets(arrTplSh(j))
                        wbImp.Sheets(Trim(arrImpSh(j))).UsedRange.Copy
    ...
    Select Case rptId
        ...
        Case "表41"
            Application.Run "'" & wbOld.Name & "'!" & moduleSub , wbOld, True, CDate(DateSerial(Left(westernMonthEnd, 4), Mid(westernMonthEnd, 5, 2), Right(westernMonthEnd, 2)))
        Case "FM2"
            Application.Run "'" & wbOld.Name & "'!" & moduleSub, wbOld
        Case "FM10"
            Application.Run "'" & wbOld.Name & "'!" & moduleSub , wbOld
        Case "F1_F2"
            Application.Run "'" & wbOld.Name & "'!" & moduleSub , wbOld, True, ROCMonth
        Case "AI240"
            Application.Run "'" & wbOld.Name & "'!" & moduleSub , wbOld, True, CDate(DateSerial(Left(westernMonthEnd, 4), Mid(westernMonthEnd, 5, 2), Right(westernMonthEnd, 2)))
    End Select
    ...
    '— 貼入申報模板 ——
    If rptId = "表41" Or rptId = "FM2" Or rptId = "FM10" Then

        ' Set wbNew  = Workbooks.Open(basePath & "\" & newTplRel)
        Set wbDecl = Workbooks.Open(declTplPathOld)
        ...
        For j = 2 To lastMap
            ...
            End If
        Next j
        wbDecl.SaveCopyAs declTplPathNew
        wbDecl.Close False
    End If
    wbNew.Close SaveChanges:=False 
...
Public Sub ImportCloseRate(ByVal csvPath As String, _
    ...
    For i = lastRow To 2 Step -1
        ...
        If IsEmpty(shCsv.Cells(i, "A").Value) Or IsEmpty(shCsv.Cells(i, "B").Value) Or IsEmpty(shCsv.Cells(i, "C").Value) Or _
           Left(shCsv.Cells(i, "A").Value, 4) = "經副襄理" Or shCsv.Cells(i, "A").Value = "TWD" Then
            isRowDelete = True
        End If
...
' Public Sub Test()
'     Dim basePath As String
'     basePath = ParentFolder(ThisWorkbook.Path)
